...
- 市场力量
- 识别客户
  - 能够他们的支付意愿将他们分成不同的群体。识别客户越精准，差别取价的能力与与越强 ← 这需要对客户行为有充分了解
...
- 低交易成本
  - 假如区别取价本身增加额外成本，就改变了厂商原有的成本结构，因而造成区别取价的失效
...
- 与单一定价独占厂商相比，福利增加或者减少无法确定
  - 消费者剩余是被掠夺
  - 区隔市场会改变成本函数与需求函数 → TPC下的总产量可能较单一定价的总产量或多或少。当总产量变得更少时，一般社会福利会更低
...
- 实施办法：由于每位消费者剩余的多少决定于单位使用费，所以厂商可以将单位使用价格 $p_x$ 设定为 $c$ ，则每位消费者将购买 $x_c$ 单位的产品X
- 此时厂商是 **没有差额利润**而每位消费者的消费剩余刚好是cae，因为厂商将**入场费A**设定为cae即可**掠夺全部消费者**
- 只要厂商能观察到消费者类型就可以采取此方案
]
.less-right[
