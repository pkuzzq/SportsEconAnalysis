...
- 在长期分析中，企业可以调整所有生产要素，包括劳动和资本。这种灵活性允许企业选择最优的要素组合来最大化产量或效率 → 意味着长期生产函数可以更好地展示两种要素之间的替代关系

- 规模报酬（Returns to Scale）： 规模报酬关注当所有生产要素的使用量同时按照某一比例增加时，产量如何变化。如果产量增加的比例等于生产要素增加的比例，那么称为恒定规模报酬；如果产量增加的比例大于生产要素增加的比例，则为递增规模报酬；反之则为递减规模报酬

- 边际报酬与生产技术：在长期生产函数中，企业可以采用不同的生产技术。每种技术对劳动和资本的边际报酬（即增加一个单位的劳动或资本所带来的额外产量）可能是不同的 <br> → 短期中，我们谈论边际报酬递减规律，是建立资本投入为固定的情况下；但长期而言，不仅需要单一要素持续投入会受边际报酬规律影响，原本的单一要素的边际产量也会受到**要素比例投入比例改变**的影响（交叉效果）
