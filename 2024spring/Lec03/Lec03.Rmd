...
- 为什么要深入生产理论？
  - 是要素市场与产品市场的连接点
  <br>生产决策是理解球队雇佣球员的关键；球队行为决定了联盟竞争程度。
  - 对理解球队决策、联盟结构、规制有帮助 
...
从成本线看供给生产函数。当经济学打开厂商生产的黑箱时，首先看到的是要素的投入，包括要素价格、要素数量以及要素组合的方式。我们将会定义出生产函数、生产技术以及生产阶段。在图形上，我们看到是L-Q平面关系

对应于不同生产函数，就会有成本函数，我们将会得到Q-$平面关系

所以接下来从生产技术出发

利用成本函数，可以讨论价格接受厂商的最适生产、销售行为
...
.pull-right[
#### 定义:生产要素投入量与.pink[产品最大产出量]之间的关系
- $Q=f(L, K)$ 。为了方便，暂且分析短期(只有一种可变要素)。
- 有时以产量(Q,output)代替产品最大产出量(TP)
- 生产行为都是在一个时间段内进行的 → TP 是**流量**概念
<br>→像土地而言，是一种**存量**概念
<br>→资本有时是固定的，有时是变动的，同时又是耐久财，不会因生产而消失。当前的资本量是过去过去新增资本累加减去耗损与折旧，因此通常也视为**存量**
]
...
- 理性的生产者既不会将生产停留在StageI段，也不会将生产扩张到StageIII
...
- 由于生产投入的要素分为**可变要素**和**固定要素**，因此对应到成本来看，也要分为**可变成本**和**固定成本**
- 一个生产要素分为可变还是固定，须视时间长短而定
...
.pull-right[
#### economies/diseconomies of scale:长期平均总成本随**产量**增加而减少的特性
- **规模经济与规模不经济** 反映了长期中厂商投入生产要素的集中程度同经济效益之间的关系
  - 规模经济：SRAC 相切于 LRAC 的下降处
- 导致 U 型原因不同
  - SR：边际报酬递减
  - LR：规模报酬递增与规模报酬递减
...
- 最适规模：能够使得 LRAC 打到最低点的规模
  - 最适规模非常大：产量扩大非常大 → LRAC 持续下降
  - 最适规模有很多 → LRAC 形状像盘子，超市等
  - 最适规模只有一个 → V 型
  
- 对于规模经济可以细分为内部经济(internal)和外部经济(external)
  - 内部经济：规模扩大后，由于厂商内部因素使得长期平均成本降低
    - 原因：更有效分工和专业化、因为资本财无法分割太小、因扩大后副产品可以使用、可大量采购原材料和零部件而成本降低
    - 效果：LRAC 呈现负斜率
  - 外部经济：规模扩大后，由于厂商以外因素造成 LRAC 持续降低
    - 原因：促成中间产品价格降低→汽车厂商扩大，中间产品价格下降
    - 效果：LRAC 整体下移

- 规模不经济
  - 内部不经济原因：管理效率下降、分工无法提升生产效率
  - 效果：LRAC 为正斜率
...
- .hi.orange[收入线：厂商要素转换为产品后，必须经由市场进行销售。如何销售?定价?与供给关系密切→从最简单开始]
